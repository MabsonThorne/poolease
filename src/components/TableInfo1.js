...
const TableInfo = ({ className = '', showMinusButton = false, tableName, refreshTableSelection }) => {
  const {
    currentPage,
    setCurrentPage,
    totalCost,
  ...
  }, [tableName, fetchCurrentOrder]);

  useEffect(() => {
    fetchOrderData();
    const intervalId = setInterval(fetchOrderData, 10000); // 每10秒刷新一次
