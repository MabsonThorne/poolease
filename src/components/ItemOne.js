...
const ItemOne = ({ className = "", propWidth, quantity = 0, showMinusButton = false, product }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row items-start justify-start py-0 px-1.5">
        <div className="relative inline-block min-w-[39px] z-[3]">{product.name}</div>
      </div>
      <div className="flex flex-row items-start justify-start py-0 px-1.5">
        <div className="relative inline-block min-w-[26px] z-[3]">￥{product.price}</div>
      </div>
      {quantity > 0 && (
